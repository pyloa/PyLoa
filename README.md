...
**or** from its latest [development release](https://github.com/) on GitHub
...
* `params["ìnstance"]`: Must define a configuration of a subclass implementation of [`pyloa.instance.InstanceGenerator`](https://gooogle.com), 
...
* `params["agent"]`: Must define a configuration of a subclass implementation of [`pyloa.agent.Agent`](https://google.com), 
...
* `params["environment"]`: Must define a configuration of a subclass implementation of [`pyloa.environment.Environment`](https://gooogle.com), 
which consumes a problem instance and let's the agent *play* until it terminates. An `environment` constitutes as a problem
definition.

A minimal example for learning the `(5,6)-paging-problem` with a [`QTableAgent`](https://google.com) on a 
[`PagingEnvironment`](https://google.com) can be invoked with
...
in [pyloa.utils.load](http://google.com)). Semantically the three different runmodes stand for: 
